            ...
            if 'content' in payload:
                ...
                if isinstance(content, str):
                    ...
                    if contains_reasoning:
                        print("[WARN] WARNING: Report may contain reasoning/thinking text")
                    else:
                    ...
                    print(f"[WARN] Content is not a string: {type(content)}")
                    print(f"Content: {str(content)[:200]}")
            
            print(f"\nGenerated at: {payload.get('generated_at', 'N/A')}")
            print(f"Agent: {payload.get('agent', 'N/A')}")
        else:
            print("[FAIL] No report data found in database")
    else:
