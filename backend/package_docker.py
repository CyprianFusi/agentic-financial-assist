...
def run_packaging(agent_name):
    ...
    print(f"\n[PKG] Packaging {agent_name.upper()} agent...")
    print(f"  Running: cd {agent_dir} && uv run package_docker.py")

    # Prepare environment - remove VIRTUAL_ENV to avoid conflicts with subdirectory venvs
    env = os.environ.copy()
    env.pop('VIRTUAL_ENV', None)

    try:
        result = subprocess.run(
            ["uv", "run", "package_docker.py"], cwd=str(agent_dir), capture_output=True,
        ...
        if result.returncode == 0:
            ...
            if zip_files:
                ...
                print(f"  [WARN] Warning: No zip file found after packaging")
                return True
        else:
            print(
