...
def check_requirements():
    ...
    # Check npm (use shell=True on Windows as npm is a .cmd file)
    try:
...
def start_frontend():
    ...
    # Check if dependencies are installed (use shell=True on Windows for npm)
    if not (frontend_dir / "node_modules").exists():
        print("  Installing frontend dependencies...")
        subprocess.run(["npm", "install"], cwd=frontend_dir, check=True, shell=True)

    # Start the frontend (use shell=True on Windows for npm)
    proc = subprocess.Popen(
        ["npm", "run", "dev"],
        cwd=frontend_dir,
        stdout=subprocess.PIPE,
