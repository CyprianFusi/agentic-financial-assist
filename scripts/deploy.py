...
def get_command(cmd):
    """Get the correct command name, handling Windows .cmd extensions."""
    if sys.platform == "win32":
        # On Windows, try the .cmd version first (for npm, etc.)
        cmd_exe = shutil.which(f"{cmd}.cmd") or shutil.which(cmd)
        return cmd_exe if cmd_exe else cmd
    return cmd
...
def run_command(cmd, cwd=None, check=True, capture_output=False, env=None):
    ...
    # On Windows, use shell=True to find .cmd files like npm.cmd
...
def check_prerequisites():
    ...
    for tool, message in tools.items():
        try:
            # Use shell=True on Windows to find .cmd files like npm.cmd
